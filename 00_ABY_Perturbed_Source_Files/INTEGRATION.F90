...
!              LOAD INITIAL STATE VECTOR
           ...
           !write(StateTime,'(F12.5,10E20.11)'),(X0(i)%E,i=1,nv)
...
!.....MAIN INTEGRATION LOOP
    ...
    DO while (.not.done)
        ...
        write(*,*);  CALL OUT_MATRIX( junk, STM1, NV, NV, NV)
   ...
   !WRITE(STM_HISTORY,*) ''
    ...
    do i=1, NV
       ...
       CALL OUT_MATRIX( STM_HISTORY, STM_1ST( 1:NV, 1:NV), NV, NV, NV)
       
       if (p_order >= 2)    then
           do i=1, NV
              !write(STM_HISTORY,*)''
               CALL OUT_MATRIX( STM_HISTORY,STM_2ND(I,1:NV,1:NV), NV, NV, NV)
           end do
       end if
       ...
       if (p_order >= 3)    then
           do i=1, NV
               do j=1,NV
                  !write(STM_HISTORY,*) ''
                   CALL OUT_MATRIX( STM_HISTORY,  STM_3RD(I,J,1:NV,1:NV), NV, NV, NV)
               end do
           end do
       end if 
       ...
       if (p_order >= 4)    then
           do i=1, NV
               do j=1,NV
                   do k=1,NV
                      !write(STM_HISTORY,*) ''
                       CALL OUT_MATRIX( STM_HISTORY,  STM_4TH(I,J,K,1:NV,1:NV), NV, NV, NV)
                   end do 
               end do
           end do
       end if  
